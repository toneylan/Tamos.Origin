    ...
    internal class WxPayApi
    {
        ...
        */
        public static WxPayData Micropay(WxPayData inputObj, int timeOut = 10)
        {
            ...
            var config = inputObj.Config;
            inputObj.SetValue("spbill_create_ip", config.GetIp());//终端ip
            inputObj.SetValue("appid", config.GetAppID());//公众账号ID
            inputObj.SetValue("mch_id", config.GetMchID());//商户号
            ...
            //将xml格式的结果转换为对象以返回
            WxPayData result = new WxPayData(config);
        ...
        */
        public static WxPayData OrderQuery(WxPayData inputObj, int timeOut = 6)
        {
            ...
            inputObj.SetValue("appid", inputObj.Config.GetAppID());//公众账号ID
            inputObj.SetValue("mch_id", inputObj.Config.GetMchID());//商户号
            ...
            string response = HttpService.Post(xml, url, false, timeOut, inputObj.Config);//调用HTTP通信接口提交数据
            ...
            //将xml格式的数据转化为对象以返回
            WxPayData result = new WxPayData(inputObj.Config);
            result.FromXml(response);
        ...
        */
        public static WxPayData Reverse(WxPayData inputObj, int timeOut = 6)
        {
            ...
            string response = HttpService.Post(xml, url, true, timeOut, inputObj.Config);
        ...
        */
        public static WxPayData Refund(WxPayData inputObj, int timeOut = 6)
        {
            ...
            string response = HttpService.Post(xml, url, true, timeOut, inputObj.Config);//调用HTTP通信接口提交数据到API
	    ...
	    */
	    public static WxPayData RefundQuery(WxPayData inputObj, int timeOut = 6)
	    {
            ...
            string response = HttpService.Post(xml, url, false, timeOut, inputObj.Config);//调用HTTP通信接口以提交数据到API
	    ...
	    */
	    public static WxPayData ShortUrl(WxPayData inputObj, int timeOut = 6)
	    {
            ...
            string response = HttpService.Post(xml, url, false, timeOut, inputObj.Config);
        ...
        */
        public static WxPayData UnifiedOrder(WxPayData inputObj, int timeOut = 6)
        {
            ...
            {
                inputObj.SetValue("notify_url", inputObj.Config.GetNotifyUrl());//异步通知url
            }

            inputObj.SetValue("appid", inputObj.Config.GetAppID());//公众账号ID
            inputObj.SetValue("mch_id", inputObj.Config.GetMchID());//商户号
            inputObj.SetValue("spbill_create_ip", inputObj.Config.GetIp());//终端ip	  	    
            inputObj.SetValue("nonce_str", GenerateNonceStr());//随机字符串
            inputObj.SetValue("sign_type", WxPayData.SIGN_TYPE_HMAC_SHA256);//签名类型
	    ...
	    */
	    ...
	    {
		    //如果不需要进行上报
		    if(inputObj.Config.GetReportLevel() == 0)
            {
			    return;
		    } 

		    //如果仅失败上报
		    if(inputObj.Config.GetReportLevel() == 1 && inputObj.IsSet("return_code") && inputObj.GetValue("return_code").ToString() == "SUCCESS" &&
		    ...
		    //上报逻辑
		    WxPayData data = new WxPayData(inputObj.Config);
	    ...
	    */
	    public static WxPayData Report(WxPayData inputObj, int timeOut = 1)
	    {
		    ...
		    inputObj.SetValue("appid",inputObj.Config.GetAppID());//公众账号ID
		    inputObj.SetValue("mch_id",inputObj.Config.GetMchID());//商户号
            inputObj.SetValue("user_ip",inputObj.Config.GetIp());//终端ip
        ...
        */
        ...
        {
            var ran = new Random();
            return string.Format("{0}{1}{2}", inputObj.Config.GetMchID(), DateTime.Now.ToString("yyyyMMddHHmmss"), ran.Next(999));
        }*/
